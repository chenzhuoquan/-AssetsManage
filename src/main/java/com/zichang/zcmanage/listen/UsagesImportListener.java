...
@Slf4j
public class UsagesImportListener extends AnalysisEventListener<UsageInfoData> {


    private final UsageInfoService usageInfoService;

    private static final Logger LOGGER = LoggerFactory.getLogger(UsagesImportListener.class);

    private static final int savaBatchSize=100;

    //临时存储数据
    private List<UsageInfo> cacheAssetsList=new ArrayList<>(savaBatchSize);

    public UsagesImportListener(UsageInfoService usageInfoService) {
        this.usageInfoService = usageInfoService;
    }

    @Override
    public void invoke(UsageInfoData usageInfoData, AnalysisContext analysisContext) {
...
/*  if(cacheAssetsList.size()>=savaBatchSize){
    ...
    public void doAfterAllAnalysed(AnalysisContext analysisContext) {
        try {
            cacheAssetsList.forEach(usageInfoService::save);
            //usageInfoService.saveBatch(cacheAssetsList);
            log.info("所有数据解析完成！");
        }  catch (Throwable e) {
            // 捕获其他异常，做通用处理
