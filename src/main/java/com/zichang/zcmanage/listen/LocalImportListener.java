...
import com.yupi.springbootinit.model.data.LocationInfoData;
...
@Slf4j
public class LocalImportListener extends AnalysisEventListener<LocationInfoData> {


    private final LocationInfoService locationInfoService;

    private static final Logger LOGGER = LoggerFactory.getLogger(LocalImportListener.class);

    private static final int savaBatchSize=100;

    //临时存储数据
    private List<LocationInfo> cacheAssetsList=new ArrayList<>(savaBatchSize);

    public LocalImportListener(LocationInfoService locationInfoService) {
        this.locationInfoService = locationInfoService;
    }

    @Override
    public void invoke(LocationInfoData usageInfoData, AnalysisContext analysisContext) {
        LocationInfo usageInfo=new LocationInfo();
        BeanUtils.copyProperties(usageInfoData,usageInfo);
        cacheAssetsList.add(usageInfo);
      */
/*  if(cacheAssetsList.size()>=savaBatchSize){
    ...
    public void doAfterAllAnalysed(AnalysisContext analysisContext) {
        try {
            locationInfoService.saveBatch(cacheAssetsList);
            log.info("所有数据解析完成！");
        }  catch (Throwable e) {
            // 捕获其他异常，做通用处理
